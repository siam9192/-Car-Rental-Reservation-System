...
const BookingForm = ({car}:TBooKingFormProps) => {
  ...
  const additionalOptions =addOptions.map((item:string)=>({display:item,value:item}))

  
  return (
    <div className='mt-5'>
         <h1 className="text-3xl font-semibold dark:text-slate-50 ">Booking Form</h1>
         <div className='mt-5'>
            <Form onSubmit={onSubmit} resolver={zodResolver(bookingValidationSchema)} reset={true}>
             <FormInput name='nid' label='NID Number ' type='text'/>
             <FormInput name='passport' label='Passport Number ' type='text'/>
             <FormSelect name='additionalOption' label='Select Additional option (optional)' options={additionalOptions}/>
             <FormInput name='drivingLicense' label='Driving License ' type='text'/><button className='mt-5 bg-secondary-color  py-3 w-full text-white'>Book</button>
            </Form>
         </div>
