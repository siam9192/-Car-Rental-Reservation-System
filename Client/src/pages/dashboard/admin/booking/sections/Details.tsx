...
import { MdCheck, MdOutlineAirlineSeatReclineExtra, MdOutlineElectricBolt } from 'react-icons/md'
...
const Details = ({id}:TDetails) => {
     const {data,isLoading:carLoading,isFetching:carFetching,refetch} = useGetCarQuery(id)
    ...
    return (
        ...
        {
            !id ?  
            <h1 className='text-4xl text-slate-50 mt-52 text-center'>Select a car</h1>
        :
        <div>
        {
            carLoading  ?   <div className="mt-32 flex flex-col items-center justify-center space-y-4 h-52">
      ...
      <div >
     <div>
     <div className="flex justify-center items-center"><img className="w-full " src={car?.images[activeImage]} alt="" /></div>
     <div className='grid grid-cols-3 gap-3 my-5 '>
     {car?.images.map((image,index)=><div onClick={()=>setActiveImage(index)} key={index} className={`border ${activeImage === index ?'border-primary-color':''}`}>
        <img src={image} alt="" />
     </div>)}
     </div>
     </div>
        ...
        <h1>
          ...
          <div className="mt-5">
            <h3 className='text-xl dark:text-slate-100 text-gray-950 mb-2'>Features:</h3>
            {car?.features.slice(0, 5).map((feature, index) => {
              return (
                <li key={index} className="flex items-center gap-1 dark:text-slate-200">
          ...
          <div className="mt-5">
            <h3 className='text-xl dark:text-slate-100 text-gray-950 mb-2'>Locations:</h3>
          ...
          <div className="mt-5">
            <h3 className='text-xl dark:text-slate-100 text-gray-950 mb-2'>Insurances:</h3>
