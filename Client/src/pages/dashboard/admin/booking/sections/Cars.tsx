...
const Card = ({car,selected}:{car:TCar,selected:boolean})=>{
 ...
 return  <div className={`bg-white dark:bg-[#1D232A] p-3 rounded-lg hover:cursor-pointer   ${selected ? 'border-2 border-primary-color':''}`}>
...
const Cars = ({setId}:TCarsProps) => {
  ...
  return (
    <div className=' col-span-12 lg:col-span-6 space-y-5 max-h-[80vh overflow-y-auto '>
      <h1 className='text-xl dark:text-slate-100 font-medium text-end'><span className='font-bold'>{cars?.length}</span> Cars Found</h1>
    {
     cars?.map((car,index)=><div onClick={()=>setActive(index)} key={index}><Card  car={car} selected={active === index}/></div>)
    }
