...
const EditProfile = () => {
  ...
  return (
    ...
    <div>
      <h3 className="text-2xl font-bold dark:text-slate-50">Update Profile</h3>
      <div className='mt-5'>
      <div className='flex justify-center items-center flex-col gap-3'>
         <img className='size-28 md:size-32 rounded-full border dark:border-none' src={newProfilePhoto?.objectUrl||user?.profilePhoto||'https://media.lordicon.com/icons/wired/flat/21-avatar.gif'} alt="" />
         <input ref={imageInputRef} onChange={profileImageInputOnChange} className='hidden' type="file" name="" id="" />
         <button onClick={openProfileImageInput} className='px-3 py-1 bg-secondary-color text-[0.9rem] text-white rounded-full'>Change Picture</button>
         </div>
     
        <Form onSubmit={onSubmit} defaultValues={defaultValues} resolver={zodResolver(updateProfileValidationSchema)}>
        <FormInput name='name' label='Name' type='text'/>
        <FormInput name='phone' label='Phone' type='text'/>
        <FormInput name='address' label='Address' type='text'/>
        <div className='mt-5 text-end'>
        <button className='px-4 py-2  bg-secondary-color text-white'>Update</button>
        </div>
        </Form>
      </div>
    </div>
