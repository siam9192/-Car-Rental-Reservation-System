...
const DashboardHeader = () => {
  ...
  return (
    <header className='py-4 md:py-6 px-5 bg-white dark:bg-dark-light-secondary fixed left-0 top-0 z-50 w-full '>
     ...
     <Link to={'/dashboard'} className="pl-4 flex items-center gap-2 rounded-full border bg-blue-100 ">
        <h2 className="font-bold uppercase">SIAM</h2>
        <img
          className="size-10 rounded-full"
          src={user?.profilePhoto||'https://media.lordicon.com/icons/wired/flat/21-avatar.gif'}
          alt=""
        />
      </Link>
      <ToggleMode/>
     <button onClick={()=>dispatch(toggleDashboardSidebar())} className='lg:hidden block text-black dark:text-white text-2xl w-fit'>
     <RiMenu3Fill />
     </button>
     </div>
     </div>
